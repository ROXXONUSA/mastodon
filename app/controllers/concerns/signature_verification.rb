...
module SignatureVerification
  ...
  def verify_signature_strength!
    raise SignatureVerificationError, 'Chatterly requires the Date header or (created) pseudo-header to be signed' unless signed_headers.include?('date') || signed_headers.include?('(created)')
    raise SignatureVerificationError, 'Chatterly requires the Digest header or (request-target) pseudo-header to be signed' unless signed_headers.include?(Request::REQUEST_TARGET) || signed_headers.include?('digest')
    raise SignatureVerificationError, 'Chatterly requires the Host header to be signed' unless signed_headers.include?('host')
    raise SignatureVerificationError, 'Chatterly requires the Digest header to be signed when doing a POST request' if request.post? && !signed_headers.include?('digest')
  end
